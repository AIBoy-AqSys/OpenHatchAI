...
### このリポジトリの性質（混同防止）
- このリポジトリは **Server Construction Kit（設計書・資材）** であり、アプリ本体のソースコード置き場ではない
- あなたの仕事は、Issue解決そのものではなく **Issue解決ができる基盤（VPS + agentd）を作ること**
- ローカル（このリポジトリ）と、ターゲットVPS上の実環境（例: `/opt/agentd`, `/etc/nginx`）を混同しない

### 作業スコープ（安全と暴走防止）
- **ローカル編集（このリポジトリを改変する場合）**：原則として `./` 配下のみを対象にし、親ディレクトリや他プロジェクトを探索しない
- **サーバ操作**：ターゲットVPS上の、構築対象パスに限定して実施する（不明点があれば「不足条件」として止めて報告する）
- 秘密情報（APIキー、Webhook secret、SSH鍵、トークン等）を **ログやIssue本文へ貼らない**
...
### 作業記録（推奨）
- このリポジトリ側の作業で、設計書やマニフェストを更新した場合は、簡単でよいので `log/[YYYYmmdd]_[short].log` に目的・変更点・確認結果を残す
